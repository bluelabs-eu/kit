...
async function render_response({
	...
	if (options.amp) {
		...
		init = `<script>window.__KIT_DATA__ = {
			target: ${options.target ? `${s$1(options.target)}` : 'body'},
			...
			hydrate: ${
				...
				nodes: [
					${(branch || []).map(({ node }) => `${s$1(node.entry)}`).join(',\n\t\t\t\t\t\t')}
				],
				legacy_nodes: [
					${(branch || []).map(({ node }) => `${s$1(node.legacy)}`).join(',\n\t\t\t\t\t\t')},
				],
				page: {
					...
					query: ${page && page.query ? s$1(page.query.toString()) : ''},
					params: ${
						page && page.params
	...
	if (options.entry_legacy) {
		legacy_scripts = [
			...
			`<script nomodule id="vite-legacy-polyfill" src=${s$1(
				options.entry_legacy.polyfills
			)}></script>`,
			`<script nomodule id="vite-legacy-entry" data-src="${s$1(
